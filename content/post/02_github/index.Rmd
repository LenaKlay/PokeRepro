...
RStudio will ask you to restart, and after that, you will observe a new tab **Git** is RStudio, and a bunch of new files and folders :

![](index_files/screen_shots/3_c.png){width=500px}



Your timeline is now ready to use ! Each time you will make a modification, it will appear in the git tab. Let's try ! We are going to change the description file to indicate what your project is about : 
...
A new window will pop, with detail of the modifications. Before taking the snapshot (before committing), write a comment so you know what modifications correspond to this new version. When that is done, you can click on the button `commit` : the snapshot is done. The git tab is clean again.
...
Another (and simpler) way to do this is to create a project from GitHub and to upload it in your computer (locally). To do so, connect to your GitHub account, go in **Repositories** tab and click to **New**. Allow GitHub to add a README file, a licence (GNU Affero General Public License v3.0) and a .gitignore (R). The new repository created on GitHub containing 3 files : a .gitignore, a LICENSE and a. README.md visible on github. Then go on **code** tab and copy the SSH link.  

Now open Rstudio and go here *file* > *new project* > *version control* > *git* past the SSH link and choose the location of the file that will contain your future project. This new project will immediately be linked to your GitHub repository. Once the project is created locally you will see 3 files: a .Rproj, the LICENSE and the README.md. However 3 others invisible files are created : a .gitignore, a .git and a .Rproj.user. They are hidden, but you can see them on R studio or GitHub and if you reveal hidden files. You can now complete your compendium by creating the different file as in the first part of this tutorial. 
